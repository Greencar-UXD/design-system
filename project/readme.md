...
- **Numbers & data** are set in **Geist Mono** with tabular figures (prices, percentages, stats, code). This is a deliberate texture cue: mono = data/precision.
...
Foundations specimen cards — **`foundations/`** (Design System tab):
...
Components — **`components/`**, organized by **atomic design** level (each dir has `*.card.html` thumbnails) — 37 components. The runtime namespace is flat (`window.DesignSystem_d67542.<Name>`) regardless of folder.
...
The Design System tab groups cards as **Atoms / Molecules / Organisms** to mirror this hierarchy.

Each component ships `<Name>.jsx` + `<Name>.d.ts` (props) + `<Name>.prompt.md` (usage). Import compiled components in HTML via `_ds_bundle.js` and `const { Button } = window.DesignSystem_d67542`.
...
For dark mode, set `data-theme="dark"` on `<html>` (or any ancestor).
